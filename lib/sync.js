...
var internals = {
  ...
  uploadFile: function (file, next) {
    ...
    var meta = { name: file.name, path: file.path, parents: [file.parent.nodeId], labels: config.labels }

    if (config.dry) {
      return next()
    } else {
      cloudDrive.uploadFile(meta, function (error) {
        if (error && error.code === 'MD5_DUPLICATE') {
...
var run = function () {
  ...
  async.waterfall([
    ...
    function (data, next) {
      cloudDrive.list({filters: 'kind:FILE AND contentProperties.contentType:(image*) AND createdBy:"z0mt3c_app_01" AND labels:' + '(' + config.labels.join(' AND ') + ')'}, function (error, results) {
